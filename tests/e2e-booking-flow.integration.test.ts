...
afterAll(async () => {
  await sql`DELETE FROM bookings WHERE booking_date IN (${testDate}, ${holidayDate}) AND (user_id = ${customerId} OR user_id = ${adminId})`
  await sql`DELETE FROM blocked_slots WHERE booking_date IN (${testDate}, ${holidayDate}) AND created_by = ${adminId}`
...
describe('end-to-end booking flow', () => {
  it('customer books, slot becomes booked, customer cancellation frees it, and it can be booked again', async () => {
    const first = await createBooking({ userId: customerId, carId: customerCarId, bookingDate: testDate, bookingTime: slot })
    expect(first.price_cents).toBe(3000)
    expect(first.status).toBe('confirmed')

    const customerView = await getSlotAvailability(testDate, 'customer')
    expect(customerView.find((item) => item.time === slot)?.state).toBe('booked')
    expect(customerView.find((item) => item.time === slot)?.available).toBe(false)
    expect(customerView.find((item) => item.time === slot)?.bookingId).toBeNull()
    ...
    expect((await getSlotAvailability(testDate, 'customer')).find((item) => item.time === slot)?.state).toBe('available')

    const second = await createBooking({ userId: customerId, carId: customerCarId, bookingDate: testDate, bookingTime: slot })
  ...
  it('rejects a blocked slot and an active holiday for customers', async () => {
    ...
    await sql`INSERT INTO blocked_slots (booking_date, booking_time, reason, created_by) VALUES (${testDate}, '16:00', 'Flow test blocked slot', ${adminId})`

    await expect(createBooking({ userId: customerId, carId: customerCarId, bookingDate: testDate, bookingTime: '16:00' }))
      .rejects.toMatchObject({ statusMessage: 'SLOT_BLOCKED' })

    await sql`INSERT INTO holidays (date, name, active) VALUES (${holidayDate}, 'Flow Test Holiday', TRUE)`
    await expect(createBooking({ userId: customerId, carId: customerCarId, bookingDate: holidayDate, bookingTime: '15:00' }))
  ...
  it('rejects banned customers and allows the same booking engine to be used by admin', async () => {
    await sql`UPDATE users SET banned = TRUE, ban_reason = 'Integration test', banned_at = now() WHERE id = ${customerId}`

    await expect(createBooking({ userId: customerId, carId: customerCarId, bookingDate: testDate, bookingTime: '17:00' }))
      .rejects.toMatchObject({ statusCode: 403, statusMessage: 'CLIENT_BANNED' })

    await sql`UPDATE users SET banned = FALSE, ban_reason = NULL, banned_at = NULL WHERE id = ${customerId}`

    const adminBooking = await createBooking({ userId: adminId, carId: adminCarId, bookingDate: testDate, bookingTime: '17:00', isAdmin: true })
    ...
    expect((await getSlotAvailability(testDate, 'admin')).find((item) => item.time === '17:00')?.state).toBe('available')
  })

  it('prevents a customer from using another customer car', async () => {
    await expect(createBooking({ userId: customerId, carId: adminCarId, bookingDate: testDate, bookingTime: '18:00' }))
