...
describe('admin booking flow integration', () => {
  ...
  it('active booking update and slot blocking are serialized by the shared date lock', async () => {
    ...
    const blockResult = sql.begin(async (tx) => {
      ...
      const activeBooking = await tx`
        ...
        WHERE booking_date = ${testDate} AND booking_time = '14:00'
    ...
    const [targetBlock] = await sql`
      ...
      WHERE booking_date = ${testDate} AND booking_time = '14:00' AND created_by = ${adminId} LIMIT 1
    `
  ...
  it('admin update and a competing booking on the old slot keep both resulting slots valid', async () => {
    ...
    const oldSlotBooking = sql.begin(async (tx) => {
      ...
      const activeBooking = await tx`
        SELECT 1 FROM bookings
        WHERE booking_date = ${testDate} AND booking_time = '10:00'
          AND status IN ('pending', 'confirmed') LIMIT 1
      `
    ...
    const [targetBooking] = await sql`
      SELECT booking_time, status FROM bookings WHERE id = ${booking.id}
    `
  ...
  it('admin update and a competing block on the old slot are serialized by the shared date lock', async () => {
    ...
    const blockResult = sql.begin(async (tx) => {
      ...
      const activeBooking = await tx`
        SELECT 1 FROM bookings
        WHERE booking_date = ${testDate} AND booking_time = '12:00'
          AND status IN ('pending', 'confirmed') LIMIT 1
      `
      if (activeBooking.length) throw new Error('ACTIVE_BOOKING_EXISTS')
      await tx`
    ...
    const [targetBlock] = await sql`
      SELECT booking_time FROM blocked_slots
      WHERE booking_date = ${testDate} AND booking_time = '12:00' AND created_by = ${adminId} LIMIT 1
    `
  ...
  it('admin update and holiday creation on the target date are serialized by the shared date lock', async () => {
    ...
    const holidayResult = sql.begin(async (tx) => {
      ...
      const activeBooking = await tx`
        SELECT 1 FROM bookings
        WHERE booking_date = ${holidayDate} AND status IN ('pending', 'confirmed') LIMIT 1
      `
      if (activeBooking.length) throw new Error('ACTIVE_BOOKING_EXISTS')
      await tx`
        INSERT INTO holidays (date, name, active)
    ...
    const [targetBooking] = await sql`
      SELECT booking_date::text AS booking_date, booking_time FROM bookings WHERE id = ${booking.id}
    `
    const [holiday] = await sql`
      SELECT date FROM holidays WHERE date = ${holidayDate} AND active = TRUE LIMIT 1
    `
  ...
  it('admin status transitions release and reacquire slots atomically with validation and audit', async () => {
    const booking = await createBooking({
      userId: customerId, carId: customerPassengerCarId, bookingDate: testDate, bookingTime: '15:00',
    })
\n    const inactive = await updateBooking({\n      bookingId: booking.id, userId: customerId, carId: customerPassengerCarId, bookingDate: testDate, bookingTime: '15:00', status: 'completed', auditActorId: adminId,\n    })\n    expect(inactive.status).toBe('completed')\n\n    const replacement = await createBooking({ userId: adminId, carId: adminCarId, bookingDate: testDate, bookingTime: '15:00', isAdmin: true })\n    expect(replacement.status).toBe('confirmed')\n    await sql\`DELETE FROM bookings WHERE id = \${replacement.id}\`\n\n    const blocked = await sql\`INSERT INTO blocked_slots (booking_date, booking_time, reason, created_by) VALUES (\${testDate}, '15:00', 'inactive-to-active validation', \${adminId}) RETURNING id\`\n    await expect(updateBooking({ bookingId: booking.id, userId: customerId, carId: customerPassengerCarId, bookingDate: testDate, bookingTime: '15:00', status: 'confirmed', auditActorId: adminId })).rejects.toMatchObject({ data: { code: 'SLOT_BLOCKED' } })\n    await sql\`DELETE FROM blocked_slots WHERE id = \${blocked[0].id}\`\n\n    await sql\`UPDATE users SET banned = TRUE, ban_reason = 'inactive-to-active validation', banned_at = now(), updated_at = now() WHERE id = \${customerId}\`\n    await expect(updateBooking({ bookingId: booking.id, userId: customerId, carId: customerPassengerCarId, bookingDate: testDate, bookingTime: '15:00', status: 'confirmed', auditActorId: adminId })).rejects.toMatchObject({ data: { code: 'CLIENT_BANNED' } })\n    await sql\`UPDATE users SET banned = FALSE, ban_reason = NULL, banned_at = NULL, updated_at = now() WHERE id = \${customerId}\`\n\n    const active = await updateBooking({ bookingId: booking.id, userId: customerId, carId: customerCrossoverCarId, bookingDate: testDate, bookingTime: '16:00', status: 'confirmed', auditActorId: adminId })\n    expect(active.status).toBe('confirmed')\n    expect(active.price_cents).toBe(3000)\n\n    const audits = await sql\`SELECT action, metadata FROM audit_logs WHERE action = 'booking.updated' AND target_id = \${booking.id} ORDER BY created_at ASC\`\n    expect(audits.length).toBe(2)\n    expect(audits[0].metadata.previousStatus).toBe('confirmed')\n    expect(audits[0].metadata.status).toBe('completed')\n    expect(audits[1].metadata.previousStatus).toBe('completed')\n    expect(audits[1].metadata.status).toBe('confirmed')\n  })\n\n
  it('admin cancellation and a competing booking on the freed slot are serialized by the date lock', async () => {
    ...
    const competingBooking = sql.begin(async (tx) => {
      ...
      const activeBooking = await tx`
        SELECT 1 FROM bookings
        WHERE booking_date = ${testDate} AND booking_time = '09:00'
          AND status IN ('pending', 'confirmed') LIMIT 1
      `
      if (activeBooking.length) throw new Error('SLOT_UNAVAILABLE')
      await tx`
  ...
  it('admin cancellation and a competing block on the freed slot are serialized by the date lock', async () => {
    ...
    const blockResult = sql.begin(async (tx) => {
      ...
      const activeBooking = await tx`
        SELECT 1 FROM bookings
        WHERE booking_date = ${testDate} AND booking_time = '18:00'
          AND status IN ('pending', 'confirmed') LIMIT 1
      `
      if (activeBooking.length) throw new Error('ACTIVE_BOOKING_EXISTS')
      await tx`
    ...
    const [block] = await sql`
      SELECT booking_date::text AS booking_date, booking_time FROM blocked_slots
      WHERE booking_date = ${testDate} AND booking_time = '18:00' AND created_by = ${adminId} LIMIT 1
    `
  ...
  it('admin cancellation reloads the booking state atomically instead of using stale data', async () => {
    ...
    const [staleSnapshot] = await sql`
      SELECT id, user_id, car_id, booking_date, booking_time, notes FROM bookings WHERE id = ${booking.id}
    `
