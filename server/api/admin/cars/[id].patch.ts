...
export default defineEventHandler(async (event) => {
  ...
  return db.begin(async (tx) => {
    const model = await tx\`SELECT v.category FROM vehicle_models v JOIN vehicle_makes m ON m.id=v.make_id WHERE m.name=\${body.make} AND v.name=\${body.model} AND m.active=true AND v.active=true LIMIT 1\`
    if (!model.length) throw createError({ statusCode: 400, statusMessage: 'INVALID_VEHICLE_MODEL', data: { code: 'INVALID_VEHICLE_MODEL' } })

    const rows = await tx\`UPDATE cars SET make=\${body.make},model=\${body.model},registration_number=\${body.registrationNumber},category=\${model[0].category},updated_at=now() WHERE id=\${id} RETURNING *\`
    ...
    await writeAuditLog({
      ...
      action: 'car.updated',
      targetId: id,
      metadata: { make: body.make, model: body.model, registrationNumber: body.registrationNumber },
    }, tx)
